...
Без имени ставятся трансформер и LightGBM, `aiw-ru models install lightgbm` ставит только лёгкую. Сравнить модели до скачивания можно командой `aiw-ru models info`: качество на корпусе, скорость, память и размер каждой; `aiw-ru models info ИМЯ` показывает одну модель подробно. Пакеты extra `ml` занимают около 200 МБ. Файлы моделей скачиваются в общий кэш `~/.cache/huggingface`. Если стоят несколько, вероятность в `scan`, `antiplagiat` и `classify` даёт самая точная из установленных; другую модель выбирает `--model`. В карточках на Hugging Face результаты на LLMTrace по жанрам, длине текста и моделям-генераторам.
...
`classify` отвечает вероятностью от необязательной модели (`--model` выбирает какой; `models info` сравнивает их до скачивания). Без установленной модели команда подскажет, как её поставить. Если модель есть, `scan` и `antiplagiat` показывают эту вероятность рядом со своей оценкой, а в JSON она лежит в поле `classifier`.
