...
<p align="center">
  <img src="docs/demo.gif" alt="Детектор aiw-ru находит 14 примет ИИ-стиля в рекламном абзаце, ставит правке 0/100, проверяет сохранность чисел, показывает, что четыре модели расходятся во мнении о правке, и оценивает главу по фрагментам в духе «Антиплагиата»" width="800">
</p>
...
- ModernBERT: дообученный русский ModernBERT в ONNX, самый точный, читает текст целиком до 8192 токенов, около 140 МБ, работает на CPU без torch ([карточка](https://huggingface.co/toiletsandpaper/russian-ai-text-detector-modernbert));
...
Без имени ставятся ModernBERT и LightGBM, `aiw-ru models install lightgbm` ставит только лёгкую, а `aiw-ru models install transformer lightgbm` — лёгкий набор для слабой машины. Сравнить модели до скачивания можно командой `aiw-ru models info`: качество на корпусе, скорость, память и размер каждой; `aiw-ru models info ИМЯ` показывает одну модель подробно. Пакеты extra `ml` занимают около 200 МБ. Файлы моделей скачиваются в общий кэш `~/.cache/huggingface`. Если стоят несколько, вероятность в `scan`, `antiplagiat` и `classify` даёт первая из установленных в порядке ModernBERT, трансформер, mini-frida, LightGBM: по доле людей, которых модель принимает за ИИ (3,4 %, 7,2 %, 9,5 % и 16,5 % на test). Другую модель выбирает `--model`. В карточках на Hugging Face результаты на LLMTrace по жанрам, длине текста и моделям-генераторам.
...
Как пользоваться классификацией, печатает `aiw-ru models guide`: какую модель взять под свою машину, как читать вероятность и итог по фрагментам длинного текста, как перепроверить отмеченные строки. То же руководство читает агент в скилле.

Модели работают через onnxruntime, а не PyTorch: колесо onnxruntime весит 24 МБ, PyTorch на Linux тянет 555 МБ и ещё 1,25 ГБ библиотек CUDA. Для старых ноутбуков и песочниц агентов без видеокарты это решает. Замеры скорости и памяти и что сделано, чтобы ONNX не отставал на длинном окне, — в [docs/runtime.md](docs/runtime.md).
...
Ещё `validate` сверяет факты (идея из [humanizer-ru](https://github.com/ilyautov/humanizer-ru)). Правка не должна снимать оговорки вроде «обычно», «примерно», «от 5000», «по-видимому» и добавлять имена, названия, аббревиатуры, числа словами и месяцы, которых нет в исходнике. Свести стопку оговорок к одной можно: «потенциально может» → «может» нарушением не считается. Снятое одиночное «может», новые слова-претензии («впервые», «всегда», «единственный») и мелкие числа словами («один», «оба») идут предупреждениями, в JSON это поле `warnings`. Код выхода они не меняют: без морфологии такие места не отличить от законной правки, их нужно сверить глазами.
...
`skill` печатает скиллы для агента, у которого нет плагина: без аргументов список, с именем SKILL.md, с именем и путём файл скилла (`references/vocabulary.md`).

`classify` отвечает вероятностью от необязательной модели (`--model` выбирает какой; `models info` сравнивает их до скачивания). Без установленной модели команда подскажет, как её поставить. Если модель есть, `scan` и `antiplagiat` показывают эту вероятность рядом со своей оценкой, а в JSON она лежит в поле `classifier` вместе с порогом. Вероятность, которая ближе 15 пунктов к порогу, помечена «близко к порогу» (`nearThreshold` в JSON): модель не уверена. `classify --all` показывает вероятности всех установленных моделей рядом и пишет, расходятся ли они. Вероятность модели — такой же сигнал, как находки правил: модели уверенно ошибаются на коротких текстах и в жанрах, которых мало в корпусе. Скилл сверяет её с находками и не правит текст ради цифры.

Трансформер и mini-frida читают одно окно в 512 токенов, это 300–350 слов от начала текста. ModernBERT читает до 8192 токенов, 4–5 тысяч слов: статью или главу диплома целиком. Если текст длиннее окна, `scan`, `antiplagiat` и `classify` пишут, что прочитала модель: «Прочитано: 351 слово из 4598, строка 1 из 56: окно модели 512 токенов», в JSON это поле `read`. Где в тексте ИИ, показывает `classify`: режет текст на фрагменты по 512 токенов по границам предложений (у ModernBERT тоже: на окне в 8192 токена ИИ-вставка тонет в человеческом тексте), соседние заходят друг на друга на четверть фрагмента, и показывает вероятность каждого фрагмента с номерами строк и итог «выше порога 2 фрагмента из 17: строки 26–28». В одно число фрагменты не усредняются: на длинных текстах LLMTrace среднее по окнам не точнее начала, а в документе, где часть написал человек и часть ИИ, среднее прячет ИИ-кусок. На M1 фрагмент занимает около 25 мс у трансформера, 70 мс у ModernBERT и 75 мс у mini-frida, чтение текста целиком у ModernBERT — до 3 секунд на 8192 токенах; текст в 40 000 знаков — около 20 фрагментов и 1–5 секунд. Замер на длинных документах из LLMTrace ([docs/long-documents.md](docs/long-documents.md)): ИИ-вставку фрагменты находят в 88–92 % документов против 13–34 % по началу, но и в целиком человеческом тексте на 20 000 знаков хоть один фрагмент выше порога бывает в 22–37 % документов. Поэтому один отмеченный фрагмент — слабый признак, два и больше — сильнее. Если проверка займёт больше 5 секунд, `classify` сразу пишет в stderr, сколько ждать. `--max-fragments N` проверяет только первые N фрагментов, `--no-fragments` оставляет одну вероятность по началу.
...
Результаты — модели pydantic: `result.to_dict()` даёт тот же JSON, что и `aiw-ru scan --json`, без поля `catalog`.

## Что ловит каталог

Полный каталог лежит в [`skills/avoid-ai-writing-russian/references/`](skills/avoid-ai-writing-russian/references/), по файлу на тему. Агент не читает его целиком: он ищет кандидатов по краткому каталогу в SKILL.md и открывает файл темы, только когда правит находку из неё. Коротко:
