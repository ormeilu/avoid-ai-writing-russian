...
Трансформер читает одно окно в 512 токенов, это 300–350 слов от начала текста. Если текст длиннее, `scan`, `antiplagiat` и `classify` пишут, что прочитала модель: «Прочитано: 351 слово из 4598, строка 1 из 56: окно модели 512 токенов», в JSON это поле `read`. Весь текст проверяет `classify`: режет его на фрагменты в одно окно по границам предложений, соседние заходят друг на друга на четверть окна, и показывает вероятность каждого фрагмента с номерами строк и итог «выше порога 2 фрагмента из 17: строки 26–28». В одно число фрагменты не усредняются: на длинных текстах LLMTrace среднее по окнам не точнее начала, а в документе, где часть написал человек и часть ИИ, среднее прячет ИИ-кусок. На M1 фрагмент занимает около 25 мс у трансформера, 65 мс у mini-frida и 110 мс у ModernBERT; текст в 40 000 знаков — около 20 фрагментов и 1–3 секунды. Если проверка займёт больше 5 секунд, `classify` сразу пишет в stderr, сколько ждать. `--max-fragments N` проверяет только первые N фрагментов, `--no-fragments` оставляет одну вероятность по началу.
