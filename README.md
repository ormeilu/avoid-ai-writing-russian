...
5. По желанию — калибровка детектора по вашим реальным отчётам.
...
`scan` выводит находки по уровням P0–P2 с номерами строк и подсказками, а также оценку 0–100 и статистику ритма. Параметр `--json` даёт машиночитаемый вывод, `--fail-above N` возвращает код 1, если оценка выше N (удобно в CI для документации).
...
`calibrate` дообучает модель `antiplagiat` на ваших отчётах. Скопируйте из отчёта фрагменты, которые система подсветила как сгенерированные, в текстовый файл, по одному на абзац через пустую строку, и передайте вместе с документом. Образцы накапливаются в `.aiw-ru.json`, и с каждым отчётом оценка точнее отражает поведение системы на ваших текстах.
