...
Кроме правил, у детектора есть две модели, обученные на русской части корпуса LLMTrace. Обе оценивают вероятность, что текст написала языковая модель:
...
Без имени ставятся обе модели, `aiw-ru models install lightgbm` ставит только лёгкую. Пакеты extra `ml` занимают около 200 МБ. Файлы моделей скачиваются в общий кэш `~/.cache/huggingface`. Если стоят обе, вероятность в `scan`, `antiplagiat` и `classify` даёт трансформер; другую модель выбирает `--model lightgbm`. В карточках на Hugging Face результаты на LLMTrace по жанрам, длине текста и моделям-генераторам.
...
`classify` отвечает вероятностью от необязательной модели, трансформера или LightGBM (`--model`). Без установленной модели команда подскажет, как её поставить. Если модель есть, `scan` и `antiplagiat` показывают эту вероятность рядом со своей оценкой, а в JSON она лежит в поле `classifier`.
