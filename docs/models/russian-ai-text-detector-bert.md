...
командой `aiw-ru models install transformer`, после чего `aiw-ru classify текст.md`
...
| [`transformer`](https://huggingface.co/toiletsandpaper/russian-ai-text-detector-bert), эта модель | $7.2\%$ | $0.9869$ | $0.945$ | $29.7$ | $39.8$ | $101.1$ |
...
- `transformer` (эта модель) — почти так же точна, лёгкая и быстрая: выбор по умолчанию.
...
| --- | --: | --: | --: | --: | --: |
| Accuracy | $0.945$ | $0.962$ | $0.948$ | $0.868$ | $0.503$ |
| ROC AUC | $0.987$ | $0.993$ | $0.991$ | $0.943$ | $0.615$ |
| ROC AUC, люди против текстов с нуля | $0.989$ | $0.994$ | $0.992$ | $0.946$ | $0.597$ |
| F1, среднее по классам | $0.943$ | $0.961$ | $0.945$ | $0.863$ | $0.466$ |
| Людей принято за ИИ | $7.2\%$ | $4.3\%$ | $9.5\%$ | $16.5\%$ | $5.9\%$ |
...
| --- | --: | --: | --: | --: | --: | --: |
| `article` | $5109$ | $5343$ | $0.948$ | $0.989$ | $0.963$ | $0.709$ |
| `factual` | $1383$ | $2209$ | $0.915$ | $0.976$ | $0.896$ | $0.673$ |
| `news` | $3326$ | $3488$ | $0.931$ | $0.984$ | $0.890$ | $0.566$ |
| `poetry` | $3561$ | $3383$ | $0.937$ | $0.979$ | $0.932$ | $0.488$ |
| `question` | $1932$ | $5894$ | $0.958$ | $0.991$ | $0.951$ | $0.729$ |
| `review` | $2678$ | $4019$ | $0.960$ | $0.990$ | $0.960$ | $0.638$ |
| `short_form` | $450$ | $3000$ | $0.934$ | $0.980$ | $0.942$ | $0.556$ |
| `story` | $2978$ | $3768$ | $0.953$ | $0.991$ | $0.967$ | $0.570$ |
...
Ниже варианты одной и той же модели. int8 идёт в поставку, если на valid теряет не больше $0.002$ ROC AUC и меняет ответ не больше чем у $0.5\%$ текстов; test в этом решении не участвует. Здесь в поставке int8: int8 меняет ответ у $0.3\%$ текстов valid.
...
метки int8 совпадают у $99.7\%$ текстов. Вывод по `inference.json`, текст за текстом, как в aiw-ru, на $300$ текстах valid даёт те же вероятности, что оценка.
...
uv run scripts/colab_transformer.py start --name tiny2 --job "train --base cointegrated/rubert-tiny2 --epochs 3 --lr 0.0001 --batch 32 --max-length 512 --evals-per-epoch 4 --patience 3"
uv run scripts/colab_transformer.py status --name tiny2
uv run scripts/colab_transformer.py fetch /content/data/transformer/runs/rubert-tiny2 ~/.cache/aiw-ru/llmtrace/transformer/full
```
...
uv run scripts/colab_transformer.py fetch /content/data/transformer/runs/rubert-tiny2/export-x86.json ~/.cache/aiw-ru/llmtrace/transformer/full/rubert-tiny2
