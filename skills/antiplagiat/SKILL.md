...
Если стоит необязательная модель LightGBM (как её предложить и поставить, описано в основном скилле, раздел «Необязательная модель»), добавь `--extra ml`: в выводе появится строка «Вероятность ИИ» по всему документу. Это отдельная оценка, а не доля ИИ-текста в отчёте «Антиплагиата», не смешивай их.
