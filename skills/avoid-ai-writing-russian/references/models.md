...
| --- | --: | --: | --: | --: | --: | --- |
| `transformer` | 0,987 | 7,2 % | 31 МБ | 210 МБ | 26 мс | по умолчанию: слабый ноутбук, песочница агента, длинные тексты |
| `modernbert` | 0,993 | 4,3 % | 138 МБ | 390 МБ | 126 мс | спорный текст, если машина позволяет |
...
На слабой машине бери `transformer`: он в 5 раз быстрее ModernBERT, а память при проверке по фрагментам не растёт с длиной текста. `--model modernbert` оставь для спорного случая: когда `transformer` у порога или когда от ответа что-то зависит и пользователь готов подождать.
