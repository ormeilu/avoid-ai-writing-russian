...
В репозитории есть детерминированный детектор на TypeScript для Bun. Он не заменяет суждение редактора: ловит только то, что ловится регулярными выражениями и статистикой ритма. Запускай его до и после правки, если Bun установлен:

```bash
bun ../../src/cli.ts scan <файл> --context vak
...
Если Bun недоступен, прямо напиши, что проверка была только модельной и детектор не запускался.

Если текст пойдёт на проверку в «Антиплагиат» и пользователь хочет оценку доли ИИ-текста по фрагментам, переключись на под-скилл [antiplagiat](../antiplagiat/SKILL.md). Для обычной чистки он не нужен.
...
**2. Проверка.** Подтверди, что перечитал файл, и скажи, осталась ли обоснованная правка. Проходы, проверки, оставленное намеренно. Если доступен Bun, запусти `validate` на версиях до и после и приведи результат.
