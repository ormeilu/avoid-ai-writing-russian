...
Кроме правил, у детектора есть две модели, обученные на русской части корпуса LLMTrace. Обе дают вероятность, что текст написала языковая модель. Трансформер (дообученный русский BERT в ONNX) точнее, LightGBM на признаках детектора легче и быстрее; если стоят обе, вероятность даёт трансформер. Вместе со скиллом они не ставятся: нужны пакеты из extra `ml` (около 200 МБ: onnxruntime, tokenizers, lightgbm, scipy, numpy, huggingface-hub) и файлы моделей с Hugging Face (около 40 МБ на обе). Проверь, стоят ли они:

```bash
uv run --project ../.. aiw-ru models --json
```

Если `ready` равно `false`, один раз за разговор предложи пользователю поставить модели: скажи, что они добавят к оценке и сколько весят, и что их вероятность — сигнал, а не доказательство авторства. Ставь только после явного согласия:

```bash
uv run --project ../.. --extra ml aiw-ru models install
```

Без имени команда ставит обе модели; `models install lightgbm` ставит только лёгкую. Если в `models` у модели есть `error`, передай пользователю, что в нём сказано (например, на macOS LightGBM нужна `brew install libomp`).
