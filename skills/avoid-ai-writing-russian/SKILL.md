...
- Весь текст проверяет `classify` (команда ниже). Он режет текст на фрагменты в одно окно по границам предложений, соседние заходят друг на друга на четверть окна, и даёт вероятность каждого. В JSON это `fragments`: `items` с номерами строк, `aboveThreshold`, `aiLines`, `aiWordShare`. Называй строки, где модель видит ИИ, а не одно число на документ. Фрагмент на стыке человеческого и ИИ-текста модель может отнести к любой стороне, а одиночный фрагмент выше порога посреди человеческого текста чаще ложное срабатывание: сверь его с находками.
- Скорость: на M1 фрагмент занимает около 25 мс у трансформера, 65 мс у mini-frida и 110 мс у ModernBERT. Пост в 40 000 знаков — около 20 фрагментов и 1–3 секунды. Текст в 330 000 знаков — около 200 фрагментов: 5 секунд у трансформера и 25 у ModernBERT. Если проверка займёт больше 5 секунд, `classify` сразу пишет в stderr, сколько ждать. Перед проверкой очень длинного текста предупреди пользователя; нужна быстрая прикидка — возьми трансформер (`--model transformer`) или ограничь проверку первыми фрагментами (`--max-fragments N`).
