...
**Длинный текст модель читает не целиком.** Трансформер видит одно окно в 512 токенов, это 300–350 слов от начала. Если текст длиннее, в выводе есть строка «Прочитано», а в JSON `read.truncated: true` и сколько слов и строк прочитано. Тогда:

- Вероятность в `scan` и `antiplagiat` относится только к началу. Скажи пользователю, какую часть прочитала модель, и не переноси это число на весь документ.
- Весь текст проверяет `classify` (команда ниже). Он режет текст на фрагменты в одно окно по границам предложений, соседние заходят друг на друга на четверть окна, и даёт вероятность каждого. В JSON это `fragments`: `items` с номерами строк, `aboveThreshold`, `aiLines`, `aiWordShare`. Называй строки, где модель видит ИИ, а не одно число на документ.
- Один фрагмент выше порога в длинном тексте — слабый признак: в целиком человеческих документах на 20 000 знаков так бывает в 22–37 % случаев, смотря по модели. Сверь его с находками `scan` в тех же строках и перепроверь по рецепту из [models.md](references/models.md). Два фрагмента и больше — признак сильнее: у человеческих документов 8–16 %, у документов со вставкой ИИ 73–86 %. Отмеченные строки шире самого ИИ-куска: фрагмент в 300 слов захватывает и человеческий текст рядом.
- Скорость: на M1 фрагмент занимает около 25 мс у трансформера, 75 мс у mini-frida и 125 мс у ModernBERT; на старом ноутбуке кратно дольше, память от длины текста не зависит. Пост в 40 000 знаков — около 20 фрагментов и 1–3 секунды. Текст в 330 000 знаков — около 200 фрагментов: 5 секунд у трансформера и 25 у ModernBERT. Если проверка займёт больше 5 секунд, `classify` сразу пишет в stderr, сколько ждать. Перед проверкой очень длинного текста предупреди пользователя; нужна быстрая прикидка — возьми трансформер (`--model transformer`) или ограничь проверку первыми фрагментами (`--max-fragments N`).
