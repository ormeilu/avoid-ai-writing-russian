...
- Экспорт ModernBERT в ONNX (`scripts/train_transformer.py export`) идёт через `scripts/fast_modernbert.py`: локальные слои считают внимание блоками в окне, а не матрицей n × n с маской, слои после последнего глобального — только для начала текста, нужного токену CLS. Ответы те же (расхождение с transformers до 3·10⁻⁶), а окно в 8192 токена на M1 считается около 2,5 секунды вместо 48. Нужно для модели с длинным окном ([#4](https://github.com/ormeilu/avoid-ai-writing-russian/issues/4)); выложенные модели не менялись.
