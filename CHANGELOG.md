...
- Четвёртая необязательная модель: дообученный `sergeyzh/rubert-mini-frida` в ONNX fp32 ([toiletsandpaper/russian-ai-text-detector-mini-frida](https://huggingface.co/toiletsandpaper/russian-ai-text-detector-mini-frida)). На test ROC AUC 0.991 и accuracy 0.948, между ModernBERT (0.993) и трансформером на rubert-tiny2 (0.987), но при пороге 50 % принимает за ИИ 9.5 % человеческих текстов против 7.2 % у трансформера. Весит около 130 МБ и в 3 раза медленнее трансформера, поэтому ставится только по имени: `aiw-ru models install mini-frida`. Порядок моделей по умолчанию: ModernBERT, mini-frida, трансформер, LightGBM.
