...
# запускает только LightGBM: torch считает в один поток (фикстура one_thread в test_transformer.py).
# Поэтому LightGBM загружается здесь, раньше тестовых модулей, и порядок их сборки не важен.
